...
namespace vks
{
	...
	*/
	void Texture2D::loadFromFile(std::string filename, VkFormat format, vks::VulkanDevice *device, VkQueue copyQueue, VkImageUsageFlags imageUsageFlags, VkImageLayout imageLayout, bool forceLinear)
	{
		...
		std::cout << "    " << "大小：" << ktxTextureSize << std::endl;
		...
		if (useStaging)
		{
			...
			std::cout << "    VkMemoryRequirements大小(StageBuffer)：" << memReqs.size << std::endl;
			...
			std::cout << "    " << "mipmap数量：" << mipLevels << std::endl;
			for (uint32_t i = 0; i < mipLevels; i++)
			{
				ktx_size_t offset;
				KTX_error_code result = ktxTexture_GetImageOffset(ktxTexture, i, 0, 0, &offset);
				std::cout << "    " << "mipmap 尺寸/Offset：" << i  << "(" << std::max(1u, ktxTexture->baseWidth >> i) <<", "<< std::max(1u, ktxTexture->baseHeight >> i) <<")" << " - " << offset << std::endl;
				assert(result == KTX_SUCCESS);
			...
			memAllocInfo.allocationSize = memReqs.size;
			std::cout << "    VkMemoryRequirements大小(VkImage)：" << memReqs.size << std::endl;
		...
		std::cout << "    " << "->> 创建采样器、创建ImageView、创建VkDescriptorImageInfo" << std::endl;
		// Create image view
