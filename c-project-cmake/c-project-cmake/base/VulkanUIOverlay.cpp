...
namespace vks
{
...
#else
		const std::string filename = getAssetPath() + "msyh.ttc";
		io.Fonts->AddFontFromFileTTF(filename.c_str(), 16.0f * scale, NULL, io.Fonts->GetGlyphRangesChineseSimplifiedCommon());
#endif
	...
	{
		bool res = ImGui::InputFloat(caption, value, step, step * 10.0f, NULL);
		if (res) { updated = true; };
		return res;
	}
