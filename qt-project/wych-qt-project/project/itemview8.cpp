...
ItemView8::ItemView8(QWidget *parent) : QWidget(parent)
{
    ...
    comboAvPage->addItem("JavLibrary");
    ...
    connect(btnOpenWiki,      &QPushButton::clicked, [ = ]() {
        ...
        if (rowIndex >= 0) {
            ...
            QDesktopServices::openUrl(QUrl(tr("http://www.javlibrary.com/cn/vl_searchbyid.php?keyword=%1").arg(code)));
        }
    });
...
void ItemView8::fileTableRowDoubleClicked(const QModelIndex& current)
{
    ...
    // 保存行号
    CacheRowIndexs[labelWorkDir->text()] = rowIndex;
}
...
void ItemView8::printList()
{
    if (openSelectFlag) {
        ...
        if (CacheRowIndexs.contains(path)) {
            fileTableView->selectRow(CacheRowIndexs[path]);
            fileTableView->scrollTo(fileTableView->currentIndex());
        }
    }
}
